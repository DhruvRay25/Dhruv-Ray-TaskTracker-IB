...
function Availability() {
    ...
    return (
        <div className='p-10'>
            <h2 className='font-bold test-2xl'>Availability</h2>
            <hr className='my-7'></hr>
            <div>
                <h2 className='font-bold my-3'>Availability Days</h2>
                <div className='grid grid-cols-2 md:grid-cols-4 gap-5'>
                    {DaysList.map((item, index) => (
                        <div key={index}>
                            <h2><Checkbox checked={daysAvailable&&daysAvailable[item?.day]?daysAvailable[item?.day]:false}
                            onCheckedChange={(e)=>onHandleChange(item.day,e)} />{item.day}</h2>
